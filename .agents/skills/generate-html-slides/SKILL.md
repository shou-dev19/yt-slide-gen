...
**導入スライドは視聴者の離脱を防ぐ最重要ポイント。デッキ内で最もリッチに、雑誌の表紙のような密度で作ること。** テロップ1行だけの薄いスライドは**禁止**。次の部品を組み合わせて情報量とインパクトを出す：
...
- **余白を残さない**：文字・アイコン・画像は大きめに。`.page-body`（既定 `space-evenly`）とフォント/行間で縦を使い切る。1ページ1〜3要素のような薄い内容でも、大きな文字・大アイコン・大ビジュアルでスカスカに見せない（目安：本文の文字は 40px 以上、見出し 56〜60px、章タイトル・CTA は 90px 以上）。詰めすぎてもいけない。
  - **ページの1/3以上が空白なら不合格**とみなし、①文字・アイコンの拡大 ②文脈に合う**いらすとや画像**（`public/images/irasutoya/`）の差し込み ③隣ページとの内容再配分、のいずれかで必ず埋める。定型スライド（注意喚起・ブログ・チャンネル登録）も同様。
