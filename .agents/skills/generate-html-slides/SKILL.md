...
> 台本または過去動画に `N-0` と `N` のペアが存在する場合、単一 `N` へ統合してはならない。`N-0` はレーダーチャート＋料金プラン、`N` は6項目ランク詳細とする。
...
2. **実行ログの機械判定を先に潰す**。キャプチャは DOM を計測して次の3点を確定的に判定し、該当があれば警告を出す（詳細は `out/slide_whitespace-report.json` と `out/slide_layout-report.json`）。目視より確実なので、警告が消えるまで直してから 3. へ進む。
   - `下端の空きが 1/3 以上のページ` … §10 の余白規則。
   - `いらすとやが主役になっているページ` … §7・§9。ページ内容に占めるイラスト面積がテキスト面積を上回るか、ページの4割以上を占めると不合格。
   - `テキストが見切れている箇所` / `内容領域からはみ出している箇所` … §10 のオーバーフロー規則。前者はブラウザが実際に文字を切っている箇所、後者は字幕帯（下部の紙面）への侵入を含む。
3. 出力された各 PNG を **画像として読み込んで実物を確認**する（Read で画像を開く、または Playwright MCP でスクショ）。全スライドを見る。機械判定で潰せない次の観点に集中する：
